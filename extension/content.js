...
(()=>{
  ...
  const seen=new Set();let enabled=false;let scheduled=0;let lastUrl='';let lastViewers=-1;let lastContext='';let lastContextSentAt=0;
  ...
  function accountContext(){
    ...
    const profileMatch=href.match(/^\/@([^/?#]+)/)||location.pathname.match(/^\/@([^/?#]+)/);
  ...
  function syncContext(){const context=accountContext();const serialized=JSON.stringify(context);const now=Date.now();if(serialized===lastContext&&now-lastContextSentAt<5000)return;lastContext=serialized;lastContextSentAt=now;chrome.runtime.sendMessage({type:'BRIDGE_CONTEXT',context}).catch(()=>{})}
