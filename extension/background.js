...
chrome.runtime.onMessage.addListener((message,_sender,sendResponse)=>{
  if(message?.type==='GET_WEB_STATE'){
    state().then(current=>sendResponse({ok:true,data:{agentConnected:current.agentConnected,agentLastError:current.agentLastError,pageHost:current.pageHost,pageType:current.pageType,tiktokLoggedIn:current.tiktokLoggedIn,tiktokContextLastSeenAt:current.tiktokContextLastSeenAt,tiktokDetectionReason:current.tiktokDetectionReason,tiktokDetected:/tiktok\.com$/i.test(current.pageHost)||/\.tiktok\.com$/i.test(current.pageHost),controllerEnabled:current.controllerEnabled}})).catch(error=>sendResponse({ok:false,error:error instanceof Error?error.message:'Extension state failed.'}));return true;
  }
