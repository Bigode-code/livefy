...
export class MediaEngine{
  ...
  constructor(readonly ffmpegPath=process.env.LIVEFY_FFMPEG_PATH||'ffmpeg',readonly ffprobePath=process.env.LIVEFY_FFPROBE_PATH||'ffprobe',now?:()=>number){this.clock=new AuthoritativeClock(now)}
  ...
  private spawnDecoder(){
    ...
    this.process=spawn(this.ffmpegPath,['-hide_banner','-loglevel','error','-re','-stream_loop','-1','-ss',position,'-i',media.path,'-map','0:v:0?','-map','0:a:0?','-vf','fps=30,scale=1080:1920:force_original_aspect_ratio=decrease,pad=1080:1920:(ow-iw)/2:(oh-ih)/2','-f','null','-'],{windowsHide:true,stdio:['pipe','pipe','pipe']});
